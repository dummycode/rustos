...
impl<'a, T: 'a> StackVec<'a, T> {
  ...
  pub fn new(storage: &'a mut [T]) -> StackVec<'a, T> {
    return StackVec { storage: storage, len: 0 };
  }
  ...
  pub fn with_len(storage: &'a mut [T], len: usize) -> StackVec<'a, T> {
    if len > storage.len() {
      panic!("oof! supplied length cannot be greater than backing length");
    }

    return StackVec { storage: storage, len: len };
  }
  ...
  pub fn into_slice(self) -> &'a mut [T] {
    return &mut self.storage[0..self.len];
  }
...
impl<'a, T: Clone + 'a> StackVec<'a, T> {
  ...
  pub fn pop(&mut self) -> Option<T> {
    ...
    let result = Some(self.storage[self.len - 1].clone());
    self.len -= 1;
...
impl<'a, T: 'a> IntoIterator for StackVec<'a, T> {
  ...
  fn into_iter(self) -> Self::IntoIter {
    return self.into_slice().into_iter();
  }
}
