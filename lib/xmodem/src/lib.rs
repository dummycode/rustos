...
impl<T: io::Read + io::Write> Xmodem<T> {
  ...
  fn expect_byte_or_cancel(&mut self, expected_byte: u8, expected: &'static str) -> io::Result<u8> {
    ...
    if byte == expected_byte {
      ...
      self.write_byte(CAN)?;
      return ioerr!(ConnectionAborted, "expected different byte but got CAN");
    } else {
      self.write_byte(CAN)?;
  ...
  pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
    if buf.len() != 128 {
      self.write_byte(CAN)?;
      return ioerr!(UnexpectedEof, "buf size not 128");
    }
    ...
    if byte != SOH {
      if byte == EOT {
        ...
        self.write_byte(CAN)?;
        return ioerr!(InvalidData, "must send SOH or EOT byte");
      }
    }

    // Read the packet number
    let packet_number = self.read_byte(false)?;
    if packet_number != self.packet {
      self.write_byte(CAN)?; // TODO send can here
      return ioerr!(InvalidData, "invalid packet number");
    }

    // Read the 1's complement number
    let ones_complement = self.read_byte(false)?;
    if packet_number + ones_complement != 255 {
      self.write_byte(CAN)?;
      return ioerr!(InvalidData, "packet number and one's complement do not add to 255");
    }
    ...
    if total != checksum {
      self.write_byte(NAK)?;
      return ioerr!(Interrupted, "checksum does not match");
    }

    // If success, return 128
  ...
  pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
    ...
    if buf.len() != 128 && buf.len() > 0 {
      return ioerr!(UnexpectedEof, "buf is incorrect length");
    }

    let mut acked = false;
    let mut attempts = 0;

    while !acked && attempts < 10 {
      self.write_byte(SOH)?;
      ...
      self.write_byte(total)?;

      let response = self.read_byte(true)?; // TODO receive can here
    ...
    if !acked  {
      return ioerr!(ConnectionAborted, "tried to send packet 10 times and failed");
    }
