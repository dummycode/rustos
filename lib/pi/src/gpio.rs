...
impl Gpio<Uninitialized> {
  ...
  pub fn into_alt(self, function: Function) -> Gpio<Alt> {
    let group = (self.pin / 10) as usize;
    let lsb = self.pin % 10 * 3;

    let mask = !(0b111 << lsb);
    self.registers.FSEL[group].write(self.registers.FSEL[group].read() & mask);

    let mask = (function as u32) << lsb;
...
impl Gpio<Output> {
  ...
  pub fn set(&mut self) {
    let group = (self.pin / 32) as usize;
    let lsb = self.pin % 32;

    self.registers.SET[group].write(1 << lsb);
  }
  ...
  pub fn clear(&mut self) {
    ...
    self.registers.CLR[group].write(1 << lsb);
  }
}
...
impl Gpio<Input> {
  ...
  pub fn level(&mut self) -> bool {
    ...
    return self.registers.LEV[group].read() & (1 << lsb) != 0;
  }
}
